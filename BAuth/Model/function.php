...
class Enginess{
    ...
    function save_performance($data) {
        ...
        $sql = "INSERT INTO model_performance (model_id, user_id, true_positive, true_negative, false_positive, false_negative, timestamp)
            VALUES (?, ?, ?, ?, ?, ?, NOW())";
        $stmt = $conn->prepare($sql);

        // Set flags based on performance type
        $tp = $tn = $fp = $fn = 0;
        if ($data['performance'] == 'true_positive') $tp = 1;
        if ($data['performance'] == 'true_negative') $tn = 1;
        if ($data['performance'] == 'false_positive') $fp = 1;
        if ($data['performance'] == 'false_negative') $fn = 1;

        if ($stmt) {
            $stmt->bind_param("ssiiii", $data['model_id'], $data['user_id'], $tp, $tn, $fp, $fn);

            if ($stmt->execute()) {
                echo "<script>alert('Test Performance Recorded Successfully!'); window.location.href='?action=test';</script>";
            } else {
    ...
    function logPrediction($model_id, $user_id, $ts, $tp, $tn, $fp, $fn) {
        ...
        $sql = "INSERT INTO model_performance (model_id, user_id, timestamp, true_positive, true_negative, false_positive, false_negative) 
                VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>{$row['status']}</td>
                        <td>
                            <a href='?action=turn_active&&model_id={$row['model_id']}'>Activate</a> |
    ...
    function list_users_by_auth_type($authType = null) {
        $conn = mysqli_connect($this->host, $this->user, $this->password, $this->DB);
    
        if ($authType) {
            $sql = "SELECT user_id, username, auth_type FROM users WHERE auth_type = ? ORDER BY user_id DESC";
            $query = $conn->prepare($sql);
            $query->bind_param("s", $authType);
        } else {
            $sql = "SELECT user_id, username, auth_type FROM users ORDER BY user_id DESC";
            $query = $conn->prepare($sql);
        }
   ...
   // Update Model
    ...
    function delete_user($table, $field, $userID){
        ...
        if ($query->execute()) {
        // echo "<script>alert('Deleted successfully!'); window.location.href='index.php?action=users';</script>";
        } else {
   ...
   function getModelPerformanceData() {
    ...
    function get_model_info($modelId) {
        $conn = mysqli_connect($this->host, $this->user, $this->password, $this->DB);

        $stmt = $conn->prepare("SELECT name, type FROM model WHERE model_id = ?");
        ...
        return $result ?: ['name' => 'Unknown', 'type' => 'Unknown'];
    }
    ...
    function get_all_models() {
        ...
        $result = $conn->query("SELECT model_id, name FROM model");
        $models = [];
        while ($row = $result->fetch_assoc()) {
            $models[] = $row;
        }
    ...
    function get_model_performance_counts($modelId) {
        $conn = mysqli_connect($this->host, $this->user, $this->password, $this->DB);
        ...
        return $result ?: ['true_positive' => 0, 'true_negative' => 0, 'false_positive' => 0, 'false_negative' => 0];
    }
